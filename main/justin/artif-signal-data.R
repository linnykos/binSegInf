...
CNA.object <- DNAcopy::CNA(cbind(coriell$Coriell.05296,coriell$Coriell.13330),
...
a = genlasso::fusedlasso1d(y.orig)
cv = genlasso::cv.trendfilter(a)

## Harvest a sparse-fused lasso
mn.elnet = genlasso::softthresh(a,lambda=cv$lambda.1se,gamma=0.05)

## Refit the segment means (because sparse fused lasso estimate is biased)
D = genlassoinf::makeDmat(length(y.orig),type="tf",ord=0)
ydiff = D %*% mn.elnet
