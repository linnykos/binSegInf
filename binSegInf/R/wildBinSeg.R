...
wildBinSeg <- function(y, thresh, numInterval = NULL,
    ...
    if(is.null(intervals)){
        intervals = generate_intervals(length(y), numInterval)
    }

    ## De-duplicate the intervals
    intervals = .deduplicate_intervals(n, intervals)
    ...
    ## Run WBS
    .wildBinSeg_inner(y, thresh, 1, n, 0, 1, verbose, env=env)
...
.wildBinSeg_inner <- function(y, thresh, s, e, j, k, verbose=FALSE, env=NULL){
    ...
    ## if(verbose) cat(j,k,fill=TRUE)
    ## if(verbose) cat(s,e,fill=TRUE)
...
#' @export
is_valid.wildBinSeg <- function(obj){
    if(!all(names(obj) %in%
            c("tree",
              "y",
              "thresh",
